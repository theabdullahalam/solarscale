...
function get_previous_planet(p_title){
    let last = 'UNKNOWN'
    let planets = get_mock_planets()
    for (let i = 0; i < planets.length; i++) {
        if (p_title === planets[i].p_title){
            if (i>0){
                last = planets[i-1].p_title
            }else{
                last = 'SUN'
            }            
        }
